...
import {
    createComentario
} from "../controllers/comentario/create_comentario.js";
import {
    getAllComentarios
} from "../controllers/comentario/get_all_comentarios.js";
import {
    getComentarioById
} from "../controllers/comentario/get_comentario_by_id.js";
import {
    updateComentario
} from "../controllers/comentario/edit_comentario.js";
import {
    deleteComentario
} from "../controllers/comentario/delete_comentario.js";
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        logGreen(`POST /api/comentarios: éxito al crear comentario ID ${newItem.id}`);
    } catch (err) {
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        ...
        if (err instanceof CustomException) {
            logRed(`Error 400 DELETE /api/comentarios/${req.params.id}:`, err.toJSON());
            return res.status(400).json(err.toJSON());
        }
        const fatal = new CustomException({
            title: "Internal Server Error",
        ...
        logRed(`Error 500 DELETE /api/comentarios/${req.params.id}:`, fatal.toJSON());
        res.status(500).json(fatal.toJSON());
    } finally {
        logPurple(
